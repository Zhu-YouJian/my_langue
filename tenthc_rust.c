...
Expr parse_primary(Parser* p) {
    if (((p->pos < Vec_len(p->tokens)) && (strcmp(((Expr*)Vec_get(p->tokens, p->pos))->kind, KIND_RBRACE()) == 0))) {
    return (Expr)make_int_expr("0");
    }
    if (((p->pos < Vec_len(p->tokens)) && (strcmp(((Expr*)Vec_get(p->tokens, p->pos))->kind, KIND_LBRACE()) == 0))) {
    return (Expr)make_int_expr("0");
    }
    if (((p->pos < Vec_len(p->tokens)) && (strcmp(((Expr*)Vec_get(p->tokens, p->pos))->kind, KIND_SEMICOLON()) == 0))) {
    parser_next(p);
    return (Expr)make_int_expr("0");
    }
    /* Let t declared=TypeParam { name: "Token" } val_ty=TypeParam { name: "Token" } */
    ...
    Expr ifv_3 = (Expr){0};
    if (((p->pos < Vec_len(p->tokens)) && (strcmp(((Expr*)Vec_get(p->tokens, p->pos))->kind, KIND_LPAREN()) == 0))) {
    parser_next(p);
    /* Let args declared=Base(Str) val_ty=Base(Str) */
    const char* args = "";
    while (((p->pos < Vec_len(p->tokens)) && (strcmp(((Expr*)Vec_get(p->tokens, p->pos))->kind, KIND_RPAREN()) != 0))) {
...
Expr parse_postfix(Parser* p) {
    ...
    void* k = ((Expr*)Vec_get(p->tokens, p->pos))->kind;
...
Expr parse_expr(Parser* p) {
    ...
    Expr ifv_1 = (Expr){0};
    if (((p->pos < Vec_len(p->tokens)) && (strcmp(((Expr*)Vec_get(p->tokens, p->pos))->kind, KIND_ASSIGN()) == 0))) {
    parser_next(p);
    /* Let right declared=TypeParam { name: "Expr" } val_ty=TypeParam { name: "Expr" } */
...
FnDef parse_fn(Parser* p) {
    ...
    int32_t ifv_4 = 0;
    if ((strcmp(((Expr*)Vec_get(p->tokens, p->pos))->kind, KIND_LBRACE()) == 0)) {
    depth = (depth + 1);
    parser_next(p);
    ifv_4 = 0;
    } else {
    ...
    int32_t ifv_3 = 0;
    if ((strcmp(((Expr*)Vec_get(p->tokens, p->pos))->kind, KIND_RBRACE()) == 0)) {
    depth = (depth - 1);
...
StructDef parse_struct(Parser* p) {
    ...
    Vec_push(fields, ({ Param* _cp = malloc(sizeof(Param)); *_cp = ((Param){ .name = fname, .type_ann = ftype }); _cp; }));
    tok = parser_next(p);
    if ((strcmp((tok).kind, KIND_COMMA()) == 0)) {
    tok = parser_next(p);
    }
...
Program parse_program(void* tokens) {
    ...
    if ((strcmp(((Expr*)Vec_get((p).tokens, (p).pos))->kind, KIND_EOF()) == 0)) {
    break;
    }
    ((strcmp(((Expr*)Vec_get((p).tokens, (p).pos))->kind, KIND_STRUCT()) == 0) ? Vec_push(structs, ({ StructDef* _t = malloc(sizeof(StructDef)); *_t = parse_struct(&p); _t; })) : ((strcmp(((Expr*)Vec_get((p).tokens, (p).pos))->kind, KIND_FN()) == 0) ? Vec_push(fns, ({ FnDef* _t = malloc(sizeof(FnDef)); *_t = parse_fn(&p); _t; })) : Vec_push(main_stmts, ({ Stmt* _t = malloc(sizeof(Stmt)); *_t = parse_stmt(&p); _t; }))));
    }
