...
FnDef parse_fn(Parser* p) {
    ...
    void* body = Vec_new();
    while (((p->pos < Vec_len(p->tokens)) && (strcmp(((Expr*)Vec_get(p->tokens, p->pos))->kind, KIND_RBRACE()) != 0))) {
    Vec_push(body, ({ Stmt _t = parse_stmt(p); &_t; }));
    }
    parser_next(p);
...
Program parse_program(void* tokens) {
    ...
    ((strcmp(((Expr*)Vec_get((p).tokens, (p).pos))->kind, KIND_STRUCT()) == 0) ? Vec_push(structs, ({ StructDef _t = parse_struct(({ Parser* _cp = malloc(sizeof(Parser)); *_cp = p; _cp; })); &_t; })) : ((strcmp(((Expr*)Vec_get((p).tokens, (p).pos))->kind, KIND_FN()) == 0) ? Vec_push(fns, ({ FnDef _t = parse_fn(({ Parser* _cp = malloc(sizeof(Parser)); *_cp = p; _cp; })); &_t; })) : Vec_push(main_stmts, ({ Stmt _t = parse_stmt(({ Parser* _cp = malloc(sizeof(Parser)); *_cp = p; _cp; })); &_t; }))));
    }
...
const char* cgen_stmt(Stmt* stmt) {
    ...
    ifv_5 = str_add(str_add(str_add(str_add("int ", stmt->name), " = "), cgen_expr(({ Expr* _cp = malloc(sizeof(Expr)); *_cp = ref_tmp_1; _cp; }))), ";\n");
    } else {
    ...
    ifv_4 = str_add(str_add("return ", cgen_expr(({ Expr* _cp = malloc(sizeof(Expr)); *_cp = ref_tmp_2; _cp; }))), ";\n");
    } else {
    /* Let ref_tmp_3 declared=TypeParam { name: "Expr" } val_ty=TypeParam { name: "Expr" } */
    Expr ref_tmp_3 = stmt_to_expr(stmt);
    ifv_4 = str_add(cgen_expr(({ Expr* _cp = malloc(sizeof(Expr)); *_cp = ref_tmp_3; _cp; })), ";\n");
    }
    ifv_5 = ifv_4;
    }
    /* Let tmp_0 declared=Base(Str) val_ty=Base(Str) */
...
const char* compile(const char* src) {
    ...
    void* tokens = lexer_tokenize(({ Lexer* _cp = malloc(sizeof(Lexer)); *_cp = lex; _cp; }));
    /* Let prog declared=TypeParam { name: "Program" } val_ty=TypeParam { name: "Program" } */
    Program prog = parse_program(tokens);
    return (const char*)cgen_program(({ Program* _cp = malloc(sizeof(Program)); *_cp = prog; _cp; }));
}
