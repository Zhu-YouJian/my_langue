...
Program parse_program(void* tokens) {
    ...
    int _ppc = 0;
    while (1) {
    if (_ppc < 10) { void* _tk = Vec_get((p).tokens, (p).pos);
        printf("DBG pp#%d pos=%lld kind=%s\n", _ppc, (long long)(p).pos, _tk ? ((Token*)_tk)->kind : "NULL"); fflush(stdout); _ppc++; }
    if (((p).pos >= Vec_len((p).tokens))) {
    printf("DBG pp break: pos>=len (%lld>=%lld)\n", (long long)(p).pos, (long long)Vec_len((p).tokens)); fflush(stdout);
    break;
    }
    if ((strcmp(((Stmt*)Vec_get((p).tokens, (p).pos))->kind, KIND_EOF()) == 0)) {
    printf("DBG pp break: EOF\n"); fflush(stdout);
    break;
    }
