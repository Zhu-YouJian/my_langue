...
Expr parse_primary(Parser* p) {
    if (((p->pos < Vec_len(p->tokens)) && (strcmp(((Token*)Vec_get(p->tokens, p->pos))->kind, KIND_RBRACE()) == 0))) {
    return (Expr)make_int_expr("0");
    }
    if (((p->pos < Vec_len(p->tokens)) && (strcmp(((Token*)Vec_get(p->tokens, p->pos))->kind, KIND_LBRACE()) == 0))) {
    return (Expr)make_int_expr("0");
    }
    if (((p->pos < Vec_len(p->tokens)) && (strcmp(((Token*)Vec_get(p->tokens, p->pos))->kind, KIND_SEMICOLON()) == 0))) {
    parser_next(p);
    return (Expr)make_int_expr("0");
    }
    /* Let t declared=TypeParam { name: "Token" } val_ty=TypeParam { name: "Token" } */
    ...
    Expr ifv_3 = (Expr){0};
    if (((p->pos < Vec_len(p->tokens)) && (strcmp(((Token*)Vec_get(p->tokens, p->pos))->kind, KIND_LPAREN()) == 0))) {
    parser_next(p);
    /* Let args declared=Base(Str) val_ty=Base(Str) */
    const char* args = "";
    while (((p->pos < Vec_len(p->tokens)) && (strcmp(((Token*)Vec_get(p->tokens, p->pos))->kind, KIND_RPAREN()) != 0))) {
    ...
    ifv_4 = ((((strcmp((t).kind, KIND_SEMICOLON()) == 0) || (strcmp((t).kind, KIND_RBRACE()) == 0)) || (strcmp((t).kind, KIND_EOF()) == 0)) ? make_int_expr("0") : make_int_expr("0"));
    }
    ifv_5 = ifv_4;
    }
    ifv_6 = ifv_5;
...
Expr parse_postfix(Parser* p) {
    ...
    void* k = ((Token*)Vec_get(p->tokens, p->pos))->kind;
...
Expr parse_expr(Parser* p) {
    ...
    Expr ifv_1 = (Expr){0};
    if (((p->pos < Vec_len(p->tokens)) && (strcmp(((Token*)Vec_get(p->tokens, p->pos))->kind, KIND_ASSIGN()) == 0))) {
    parser_next(p);
    /* Let right declared=TypeParam { name: "Expr" } val_ty=TypeParam { name: "Expr" } */
...
FnDef parse_fn(Parser* p) {
    ...
    void* body = Vec_new();
    /* Let depth declared=Base(I32) val_ty=Base(I32) */
    int32_t depth = 1;
    while (((p->pos < Vec_len(p->tokens)) && (depth > 0))) {
    /* Let ifv_4 declared=Base(I32) val_ty=Base(I32) */
    int32_t ifv_4 = 0;
    if ((strcmp(((Token*)Vec_get(p->tokens, p->pos))->kind, KIND_LBRACE()) == 0)) {
    depth = (depth + 1);
    parser_next(p);
    ifv_4 = 0;
    } else {
    /* Let ifv_3 declared=Base(I32) val_ty=Base(I32) */
    int32_t ifv_3 = 0;
    if ((strcmp(((Token*)Vec_get(p->tokens, p->pos))->kind, KIND_RBRACE()) == 0)) {
    depth = (depth - 1);
...
Program parse_program(void* tokens) {
    ...
    int _ppc=0; int64_t _last_pos=0;
    while (1) {
    if(_ppc<100){void*_tk=Vec_get((p).tokens,(p).pos);
        const char* k=_tk?((Token*)_tk)->kind:"NULL";
        if(!_tk||!k||(uintptr_t)k<0x1000){printf("DBG CORRUPT at pp#%d p=%lld tk=%p k=%p\n",_ppc,(long long)(p).pos,_tk,(void*)k);
            printf("  last pos=%lld -> tk at %lld: %p\n",(long long)_last_pos,(long long)(p).pos,Vec_get((p).tokens,(p).pos));
            fflush(stdout);}
        else if(_ppc<30||_ppc>60)printf("DBG pp#%d p=%lld k=%s\n",_ppc,(long long)(p).pos,k);_ppc++;_last_pos=(p).pos;}
    if(((p).pos >= Vec_len((p).tokens))){
    break;
    }
    if ((strcmp(((Token*)Vec_get((p).tokens, (p).pos))->kind, KIND_EOF()) == 0)) {
    break;
    }
    ((strcmp(((Token*)Vec_get((p).tokens, (p).pos))->kind, KIND_STRUCT()) == 0) ? Vec_push(structs, ({ StructDef* _t = malloc(sizeof(StructDef)); *_t = parse_struct(&p); _t; })) : ((strcmp(((Token*)Vec_get((p).tokens, (p).pos))->kind, KIND_FN()) == 0) ? Vec_push(fns, ({ FnDef* _t = malloc(sizeof(FnDef)); *_t = parse_fn(&p); _t; })) : Vec_push(main_stmts, ({ Stmt* _t = malloc(sizeof(Stmt)); *_t = parse_stmt(&p); _t; }))));
    }
...
const char* cgen_fn(FnDef* f) {
    ...
    out = str_add(str_add(out, "int "), ((Param*)p)->name);
    i = (i + 1);
    0;
    }
    out = str_add(out, ") {\n");
