...
- **不可替代性（Phase 1 已实现）**：编译期 shape 检查——`Tensor[f64, 3, 4] @ Tensor[f64, 5, 6]` 在编译期报错，而非运行时崩溃。**注：Phase 1 覆盖 matmul/broadcast/构造函数 shape 推断；符号维度 unify 留待 Phase 2**
...
| **阶段 3：不可替代** | 编译期张量 shape 检查 | shape 不匹配在编译期报错，而非运行时 | ⚠️ Phase 1 已实现 |
