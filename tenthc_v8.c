...
Expr parse_primary(Parser* p) {
    if (((p->pos < Vec_len(p->tokens)) && (strcmp(((Stmt*)Vec_get(p->tokens, p->pos))->kind, KIND_RBRACE()) == 0))) {
    return (Expr)make_int_expr("0");
    }
    if (((p->pos < Vec_len(p->tokens)) && (strcmp(((Stmt*)Vec_get(p->tokens, p->pos))->kind, KIND_SEMICOLON()) == 0))) {
    parser_next(p);
    return (Expr)make_int_expr("0");
    }
    /* Let t declared=TypeParam { name: "Token" } val_ty=TypeParam { name: "Token" } */
...
FnDef parse_fn(Parser* p) {
    ...
    int32_t ifv_4 = 0;
    if ((strcmp(((Stmt*)Vec_get(p->tokens, p->pos))->kind, KIND_LBRACE()) == 0)) {
    depth = (depth + 1);
    parser_next(p);
    ifv_4 = 0;
    } else {
    ...
    int32_t ifv_3 = 0;
    if ((strcmp(((Stmt*)Vec_get(p->tokens, p->pos))->kind, KIND_RBRACE()) == 0)) {
    depth = (depth - 1);
...
Program parse_program(void* tokens) {
    ...
    int _ppc=0;
    while (1) {
    if(_ppc<200){void*_tk=Vec_get((p).tokens,(p).pos);printf("DBG pp#%d p=%lld k=%s\n",_ppc,(long long)(p).pos,_tk?((Token*)_tk)->kind:"NULL");fflush(stdout);_ppc++;}
    if (((p).pos >= Vec_len((p).tokens))) {
    break;
    }
