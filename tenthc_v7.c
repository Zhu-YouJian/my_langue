...
FnDef parse_fn(Parser* p) {
    ...
    /* Let ifv_4 declared=TypeParam { name: "Token" } val_ty=TypeParam { name: "Token" } */
    Token ifv_4 = (Token){0};
    if ((strcmp(((Stmt*)Vec_get(p->tokens, p->pos))->kind, KIND_LBRACE()) == 0)) {
    depth = (depth + 1);
    ifv_4 = parser_next(p);
    } else {
    /* Let ifv_3 declared=Unknown val_ty=Unknown */
    void* ifv_3 = 0;
    ...
    ifv_3 = Vec_push(body, ({ Stmt* _t = malloc(sizeof(Stmt)); *_t = parse_stmt(p); _t; }));
    }
    ifv_4 = *(Token*)ifv_3;
    }
    /* Let tmp_1 declared=Unknown val_ty=Unknown */
    void* tmp_1 = ifv_4;
    tmp_1;
    }
    parser_next(p);
