...
// Debug helper — auto-flushing printf
#define DBG(fmt, ...) do { printf("DBG " fmt "\n", ##__VA_ARGS__); fflush(stdout); } while(0)
...
void* lexer_tokenize(Lexer* lexer) {
    ...
    int _loop_cnt = 0;
    while ((!done)) {
    if (++_loop_cnt <= 5 || _loop_cnt % 100 == 0) DBG("lexer_tokenize: loop %d, tokens len=%lld", _loop_cnt, (long long)Vec_len(tokens));
...
Program parse_program(void* tokens) {
    ...
    ((strcmp(((Token*)Vec_get((p).tokens, (p).pos))->kind, KIND_STRUCT()) == 0) ? Vec_push(structs, ({ StructDef _t = parse_struct(&p); &_t; })) : ((strcmp(((Token*)Vec_get((p).tokens, (p).pos))->kind, KIND_FN()) == 0) ? Vec_push(fns, ({ FnDef _t = parse_fn(&p); &_t; })) : Vec_push(main_stmts, ({ Stmt _t = parse_stmt(&p); &_t; }))));
    }
...
int main() {
    ...
    DBG("main: srclen=%d", src ? (int)strlen(src) : -1);
    /* Let c declared=Base(Str) val_ty=Base(Str) */
    const char* c = compile(src);
    DBG("main: compile returned, len=%d", c ? (int)strlen(c) : -1);
