...
- **差异化能力**：编译期 shape 检查等「差异化特性」（见 §1.3）把一类在主流框架中只能**运行时**暴露的错误提前到**编译期**。
...
Tenth 的差异化能力由以下「核心特性」构成，均为**已实现**特性（覆盖范围与边界见对应章节）：
...
5. **运行时自动微分 shape 校验**：`backward()` 梯度 shape 不匹配运行时报错（手册 §11.5）。
...
- **当前版本**：v1.0.0（`tenth/Cargo.toml`）。
- **测试基线**：见 `能力梳理/能力全梳理.md` §统计基线（测试数、实例数以其为准；实例双路径可运行）；自举编译器（tenthc）验证 `[OK]`。
- **执行管线**：`.th → 词法 → 语法 → HIR → VM（默认）/ 解释器（`TENTH_NO_VM=1`）/ WASM / JIT`。

### 1.4.1 规范定稿声明（面向 v1.0.0）

> **本规范自 v1.0.0 起为对外的正式语言规范（稳定承诺）**：第 2–6 章所描述的语法与语义（词法、类型系统、所有权与借用、表达式求值、错误模型）在 **v1.0.0 起视为稳定契约**——任何破坏性语法/语义变更必须 bump **major** 并在发布说明中显式声明；兼容性新增（新语法糖、新类型、新原语）走 **minor**。
> 0.x 阶段（v0.5.0 及以前）不承诺向后兼容，历史破坏性修订已登记在《语言参考手册》相应章节与发布说明中；自 v1.0.0 起本规范为稳定契约，变更流程见 `docs/API冻结清单.md` §4。
> 规范与实现的对应关系由附录 B 测试锚点守护；发现规范与实现不一致 → 按本文档流程修订。

- **兼容性承诺**：本规范描述 v1.0.0 现状；语义变更必须同步更新本规范与《语言参考手册》。
- **API 冻结与 semver 策略**：语言语法 / 标准库 / CLI / native 的权威冻结清单 + 1.0 起 `patch=bugfix / minor=兼容新增 / major=破坏` 的版本承诺，见 `docs/API冻结清单.md`。
...
2. **无后缀且超出 i32 范围**（$< -2^{31}$ 或 $> 2^{31}-1$）时**自动提升为 i64**（不报错）；✅ **2026-09-17 已达成**（实测不再报错）；混合整数运算按**宽度优先**公共 dtype 且**可交换**，超出公共 dtype 范围时运行期响亮报错（`AUDIT-11.4.53` 已修复）；
...
> 测试锚点：`string_encoding_test.rs`（`\u{...}` 与 `\xNN` 转义，对应内部缺陷跟踪编号 AUDIT-11.4.25）。
...
| 类别 | 类型 | 说明 |
|------|------|------|
| 有符号整数 | `i8` `i16` `i32` `i64` | dtype 全程保留（词法→AST→HIR→**字节码**→Value 五层）✅ **2026-09-17 已达成**：`Op::PushInt` 携带 dtype tag（8B 值 + 1B tag），后缀字面量（`42u8`）与 `i64` 算术均按声明 dtype 生效（`AUDIT-11.4.53` 已修复） |
...
- $m$ 满足交换律、幂等律，但**不满足结合律**（该性质已形式化论证）——因此实现采用 **n 元共识语义**：同一多重集内所有非 Any 输入相同时取该值，否则取 Any（消除折叠顺序依赖）。
...
| cross_entropy / reshape 反向 | 梯度 shape 检查（反向 shape 校验的深化） | 编译期 + 运行时 |

**跨函数 shape 组合**：

- 函数体的精确 shape 传播到调用方（`fn make() -> Tensor[f64, ..] { zeros(3,4) }` → 调用方拿 `[3,4]`）；
- 多 return 路径 n 元共识合并（§3.3.2）；维度数不同 → 报错；递归函数跳过（用注解签名）；
- **前向引用 shape 精化**（2026-08-03 落地）：`lower_program` 尾部做**无环全量不动点**（`run_forward_ref_shape_fixpoint`，$2n+1$ 轮上限兜底），使「main 先于 make 定义」也拿到 body 精化 shape（顺序无关）；`main` 先于 `make` 且 shape 冲突 → 编译期拦截。
...
| 参数 shape 一致化（shape 代数求解器） | ❌ 未实现（规划中） |
| 符号维度跨函数 unify 求解 | ⚠️ 部分（同名等价 + 实参代换已实现；完整约束求解未实现） |
| 秩多态（`..` 任意秩的一等特性） | ❌ 未实现（归入 shape 检查路线图；注解 `Tensor[f64, ..]` 可解析） |
...
| 特性 | 状态 | 出处 |
|------|------|------|
| 高阶类型 HKT（`Functor<F>`） | ❌ 未实现 | 远期研究 |
| GAT（泛型关联类型） | ❌ 未实现 | 远期研究 |
| Const 泛型 | ❌ 未实现 | 归入 shape 检查路线图 |
| 协变 / 逆变 | ❌ 未实现 | 远期研究 |
| 秩多态（`Tensor[f64, ..]` 一等特性） | ❌ 未实现（注解可解析） | 归入 shape 检查路线图 |
| 生命周期省略 | ❌ 未实现 | 无显式 lifetime 语义检查 |
| 过程宏 / 语法扩展插件 | ❌ 未实现 | 需编译插件机制 |
...
- **已知边界（如实）**：跨语句的引用释放与 Deref 别名检查存在 Tenth 接受但违反别名规则的程序（`lower_stmt.rs`/`lower_expr.rs` 调用 `release_borrows` 过早释放，Deref 路径不检查别名）。这是已登记的已知缺陷（内部缺陷跟踪编号 AUDIT-11.1，见 §7.3 已知边界清单）。
...
- **函数边界释放**：函数内 `let r = Rc::new(42); Weak::new(r)` 返回后 `Rc` 正确释放、`upgrade` → `None`（VM = 解释器一致）；
...
- **Tensor 迭代语义**：按第一维遍历；1D 张量元素形态统一为**标量 Float**（VM = 解释器一致）；
...
4. **true letrec**：递归闭包自引用绑定 = **实例级可变 cell**（`Value::Shared(Rc<RefCell<Value>>)`）捕获（HIR `Closure.self_refs` + opcode `MakeCell(63)` / `BindSelfCapture(64)`）——逃逸定义作用域 + 多实例共存不产生别名静默错值（`make_fact(2,10)` / `make_fact(3,1)` = 762769，VM=解释器）；
...
3. 每任务独立 `Frame.operand_stack`（真正的协程调度）；
4. **边界（如实）**：异步机制在 VM 路径完整；解释器/WASM/JIT 路径对 `async`/`await`/`spawn` 的支持有限（详见《语言参考手册》相关章节）；`task`/`shard`/`node` 关键字预留未启用（§2.4）。
...
5. **一致性**：`?` 遇 `Err` 时，VM 与解释器均返回**单层** `Result::Err(e)`（不 double-wrap）。解释器函数边界 `unwrap_return`（core.rs）与 try 块捕获处（eval.rs）均识别 TryPropagate 携带的已是完整 `Result::Err` 值而直接透传；历史不一致已修复（内部缺陷跟踪编号 AUDIT-11.4.33），守护测试见 §5.15 测试锚点（多层 `?` + try 块单层断言）；
...
- 运行时错误定位：VM 通过行号表（`Chunk::note_line`）按指令偏移定位；JIT translator 写 `vm.current_line` 对齐。
...
- 运行时错误：解释器按 AST 节点携带位置；**VM 通过行号表定位**（`Chunk::note_line` 在语句/表达式边界记录指令偏移 → 源码行），错误按当前 `ip` 查最近前驱条目；JIT 路径 translator 写 `vm.current_line` 对齐；
...
1. ~~**`?` 解释器 double-wrap 不一致**~~：✅ 已修复（内部缺陷跟踪编号 AUDIT-11.4.33）——`Err` 时解释器函数边界与 try 块捕获均保持单层 `Result::Err(e)`，与 VM 一致（§5.15 第 5 条）；
2. **NaN 比较静默**（内部缺陷跟踪编号 AUDIT-11.4.16）：`NaN == NaN` → false 等静默通过，无提示（lossy 只传播 PossibleNaN 不报）；已登记已知限制；
3. **静默错值系统性风险**：历史上「响亮报错 → 打通路径后变静默错误值」是最危险退化方向（闭包捕获全局名 hack、chunk 名冲突、索引表错用三类均已修复并有对拍守护）；
4. **借用检查 unsoundness**（内部缺陷跟踪编号 AUDIT-11.1 B6）：§4.5，已登记；
5. ~~**错误处理覆盖：`db_query().len()` 误当成功值使用（「误用」类）尚未拦截**~~：✅ 已实现（2026-08-04，`check_silent_failure_misuse` / `check_silent_failure_misuse_method`）——Result/Option 在**非消费上下文**被使用（方法调用（白名单外，当前为空）、索引、字段访问、算术 Add/Sub/Mul/Div/Mod）→ 编译期 `TenthWarning`（带行/列/源码定位）；**不触发**：match scrutinee / `?` / try 块内 / or_die / assume_ok 参数、函数返回 / let 绑定 / 容器存储（传输合法）、调用实参、比较 / 逻辑运算；**保守边界**：跳过裸 `Type::Enum("Option")`（`Vec.get()/pop()` 静态误标为 Option 但运行时返回裸元素，`parts.get(i).trim()` 依赖——宁可少报不误报）；守护测试 `silent_failure_test.rs`（44 项，含误用拦截 +26 项）；
...
| 过程宏 / 语法扩展插件 | 需编译插件机制 |
| 协变 / 逆变 | 远期研究 |
| 生命周期省略 | 随显式 lifetime 语义 |
| ~~`tenthpm` / LSP 完整实现~~ | ✅ 已实现（tenthpm 与 LSP 均已完整实现）——此条目已从「不实现」移除 |
...
| Const 泛型 | ❌ 远期 | 归入 shape 检查路线图 |
| 秩多态（`Tensor[f64, ..]` 一等特性） | ❌ 远期 | 归入 shape 检查路线图；注解可解析（Dim::Any） |
| 完整 typestate 状态机验证 | ❌ 远期 | 当前为状态参数 + 方法过滤 + 调用点检查（§3.14） |
| shape 代数求解器 | ❌ 规划中 | 见 §3.3.5 |
| 递归程序 shape 精化（SCC 感知） | ❌ 候选 | 前向引用 shape 精化的后续候选 |
| 完整生命周期语义检查 | ❌ 远期 | 当前为语句粒度保守近似（§4.5） |
| GPU 真正落地 | ❌ 未接 CUDA Runtime | 仅 CUDA C 代码生成 + 模拟设备（内部缺陷跟踪编号 AUDIT-11.4.6） |

## 7.3 已知边界清单（如实）

| 边界 | 影响 |
|------|------|
| 借用检查语句粒度（B6 unsoundness，内部缺陷跟踪编号 AUDIT-11.1） | 部分违反别名的程序可能通过检查（§4.5） |
| ~~`?` 解释器 double-wrap~~（内部缺陷跟踪编号 AUDIT-11.4.33，已修复） | 解释器与 VM 在 `Err` 传播上已一致（单层） |
...
| NaN 比较静默（内部缺陷跟踪编号 AUDIT-11.4.16） | 条件判断错误静默 |
...
> ~~手册修订建议~~：✅ 已全部落实（2026-08-03）——补 §2.3 字面量、§2.2 `yield`、§5.12 优先级（比较分组）、§5.13 try/`?` 与 §7.4/§7.5 async/yield 专门章节、语言定位 `@` 伪语法澄清；同步修复实现缺口（十进制下划线、字节串管线、`?` try 块 double-wrap）。
...
| 前向引用 shape 精化 | `forward_ref_shape_test`（26 项） |
