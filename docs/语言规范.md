...
> 0.x 阶段（v0.5.0 及以前）不承诺向后兼容，破坏性修订已在 MEMO 与手册/规范中显式登记；自 v1.0.0 起本规范为稳定契约，变更流程见 `docs/API冻结清单.md` §4。
> 规范与实现的对应关系由附录 B 测试锚点守护；发现规范与实现不一致 → 登记 AUDIT 并按本文档流程修订。

- **兼容性承诺**：本规范描述 v1.0.0 现状；语义变更必须同步更新本规范与《语言参考手册》。
- **API 冻结与 semver 策略**：语言语法 / 标准库 / CLI / native 的权威冻结清单 + 1.0 起 `patch=bugfix / minor=兼容新增 / major=破坏` 的版本承诺，见 `docs/API冻结清单.md`（M5.1）。
...
| ~~`tenthpm` / LSP 完整实现~~ | ✅ 已实现（M4.1 tenthpm 完整 + M4.2 LSP 补强，见能力全梳理 §2.3/§2.4）——此条目已从「不实现」移除 |
