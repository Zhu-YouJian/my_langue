...
> v0.5.0 → 1.0 之间的变更仍以本规范为准，但 **0.x 阶段不承诺向后兼容**（允许破坏性修订，须同步本规范与《语言参考手册》）。
> 规范与实现的对应关系由附录 B 测试锚点守护；发现规范与实现不一致 → 登记 AUDIT 并按本文档流程修订。

- **兼容性承诺**：本规范描述 v0.5.0 现状；语义变更必须同步更新本规范与《语言参考手册》。
