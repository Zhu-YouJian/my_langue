...
2. **无后缀且超出 i32 范围**（$< -2^{31}$ 或 $> 2^{31}-1$）时**自动提升为 i64**（不报错）；⚠️ **实现现状（2026-09-17）**：实测报「整数运算结果 … 溢出 i32 范围」并中止（`AUDIT-11.4.53`）——本条承诺保留，实现补齐已排为必修；
...
| 类别 | 类型 | 说明 |
|------|------|------|
| 有符号整数 | `i8` `i16` `i32` `i64` | dtype 全程保留（词法→AST→HIR→Value 四层）⚠️ **实现现状（2026-09-17）**：dtype 未贯穿到字节码（`AUDIT-11.4.45` A / `AUDIT-11.4.53`：`42u8` 运行时为 I32、`i64` 算术不可用）——**本条承诺保留，实现补齐已排为必修** |
