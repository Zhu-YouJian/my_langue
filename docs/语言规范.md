...
> ⚠️ 现状提示：`yield` 为活跃关键字（`yield` / `yield expr`，§5.14），但《语言参考手册》§2.2 关键字表遗漏 `yield`，将在手册修订中补齐。
...
1. **关系比较（`< > <= >=`）绑定比相等（`== !=`）更紧**：`a == b < c` 解析为 `a == (b < c)`。⚠️ 这是实现现状（`binop_precedence`：Lt/Gt/LtEq/GtEq=3，EqEq/NotEq=2）；《语言参考手册》§5.12 将六种比较列在同一层级，**与实现不一致**，以本规范为准（手册修订中）。
...
> 测试锚点：`operator_precedence_test.rs`；手册 §5.12 与实现的差异见本规范 §2.7 第 1 条。
...
5. **已知不一致（如实）**：`?` 遇 `Err` 时，**VM 路径返回单层 `Result::Err(e)`，解释器路径返回双重包装 `Result::Err(Result::Err(e))`**——解释器与 VM 行为不一致的实现 bug（披露于 `try_operator_test.rs` 注释，未登记 AUDIT，建议后续登记修复）；
...
1. **`?` 解释器 double-wrap 不一致**：`Err` 时解释器返回 `Result::Err(Result::Err(e))`，VM 返回单层（§5.15 第 5 条；披露于 `try_operator_test.rs`，建议登记 AUDIT）；
...
| `?` 解释器 double-wrap（§5.15） | 解释器与 VM 在 `Err` 传播上不一致 |
...
| §5.13/5.14 异步/yield | 手册（暂缺专门章节） | 规范先行，手册待补 |
| §5.15 try/`?` | 手册 §10.6（提及） | 规范先行，手册待补 |
| §6 错误模型 | 手册 §10.5 lossy、§10.6 静默失败、§10.9 零除数 | 错误分类正式化 |
| §6.6/6.7 | 手册 §12.4 Option/Result | 一致 |

> 手册修订建议（待文档部排期）：补 §2.3 字面量、§2.2 `yield`、§5.12 优先级（比较分组）、§5.15/§5.13 专门章节、语言定位 `@` 伪语法澄清。
