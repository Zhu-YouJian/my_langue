...
| 层 3 | lossy（算错可能性格） | ⚠️ **M1 局部传播已实现**；跨函数污点（M2）未实现 | 格是链、方案 C 旁路分析；`lossy_lattice_test.rs` 9 项 |
...
**一句话给编译器实现者**：函子化不是重写编译器，而是**逐步收编**——把每个 pass 里的全局可变状态（收集器、顺序依赖）换成"纯结构递归 + 局部传播 + 必要时全局不动点"。阶段 0 已收编 3 处（跨函数 shape、backward_shapes、层 1 丢弃警告），下一个候选是层 3 的污点旁路分析（M2）。
...
### 3.3 层 3 lossy = 一个函子（⚠️ M1 局部传播已实现；M2 跨函数未实现）
...
- **M2 未实现**（设计已给，阶段 2b §6）：`lossy` 关键字（lexer/parser/HIR 双侧同步 + 语言参考手册）；**方案 C 旁路分析**（`Lowerer.var_taint: HashMap<String, Lossiness>` + `expr_taint()` 结构递归，`Type`/HIR/bytecode/wasm 零改动）；使用点检查（污点非 Exact 的值被当 Exact 用 → 报错要求 `lossy()`）；**函数返回污点传播**（函数体返回污点记录在 fn_def，调用点合并，`scope.rs` fn 表加字段）。工作量估计：约 1.5-2 人日。
- **M3 未实现**（设计要点）：与 shape 检查协同（shape 全已知时张量级判零精确化）；`backward_shapes.rs` 已函子化（阶段 0），污点传播可复用同样纯递归结构。

**函子性**：$\tau: \mathbf{Prog} \to L$ 的传播规则是**逐节点局部**的（结构递归）——这是"分析 = 函子"最纯粹的例子，组合律在表达式树局部天然成立。但**函子的跨函数组合（调用点合并被调函数返回污点）依赖 M2 的函数返回污点传播，未实现**——因此当前 $\Phi_{\text{lossy}}$ 只在"表达式树内"是函子，跨函数边界尚未接通。诚实标注。
...
| 3 | lossy | 链式格 $L$（4 元） | $\tau(\text{op}) = t_l \sqcup t_r \sqcup \text{op\_effect}$ | ⚠️ M1 表达式内；M2 跨函数未实现 | `lossy_lattice_test.rs` 9 项；`types.rs:1135/1154` |
...
2. **层 3 跨函数传播未实现**：$\Phi_{\text{lossy}}$ 当前只在表达式树内是函子；调用点合并被调函数返回污点依赖 M2（未实现）。"函子组合性"在 lossy 层目前只到表达式局部。
3. **层 2 完全未实现**：$\Phi_{\text{state}}$ 是蓝图；Tenth 现不能支撑 typestate（G1-G6）。本文档对层 2 的描述是"目标规范"，不是"现状"。
4. **不动点收敛性证明未完成**：断点 4.2 的修复方向（全局不动点）依赖"信息只增不减、冲突坍缩到 Any 为终态"的不变量；其形式化收敛证明（格论意义）列为阶段 1 前置工作，本文档不宣称已证。
...
3. **实施建议（按序）**：
   - 层 3 M2：`lossy` 关键字 + 方案 C 旁路分析 + 函数返回污点传播（打通 lossy 函子的跨函数组合）；先只对 **Lossy**（隐式降级）使用点报错，NaN/溢出保持静态错误 + 传播机制（防误报，阶段 2b §6 范围建议）。
   - 断点 4.1：调用点实参代换（Symbol → 实参维度），复用 `substitute_type` 模式；需新增测试（`zeros(a,b)` 被 `make(3,4)` 调用后与 `[5,6]` 比较）。
   - 断点 4.2：全局不动点重推导签名；前置完成收敛性论证。
