...
> **版本**：v0.5.0 → 面向 v1.0.0 | **日期**：2026-08-04
...
3. **0.x 阶段（当前 v0.5.0 → 1.0）**：minor 位允许包含破坏性修订（SemVer 0.x 规则），但**任何破坏性变更仍需**在 MEMO 与手册/规范中显式登记——0.x 的「冻结承诺」是文档纪律而非版本号纪律；
...
## 5. v1.0.0 门槛检查（遗留登记，M5.1）

以下为 M1–M4 如实登记的、**1.0 前需决策或排期**的项（不作为 1.0 冻结的阻塞项，仅登记）：

| # | 事项 | 状态 |
|---|------|------|
| 1 | 手册/规范版本已对齐 v0.5.0（本次 M5.1） | ✅ 本次完成 |
| 2 | `db_query().len()` 误用拦截已实现（M3.4） | ✅ 已实现 |
| 3 | tenthpm/LSP 完整实现（M4.1/M4.2） | ✅ 已实现 |
| 4 | AUDIT-11.4.34（VM match tuple 模式 + guard 回退错乱） | ⚠️ 待排期 |
| 5 | AUDIT-11.1（借用检查语句粒度 B6 unsoundness） | ⚠️ 已登记，1.0 前评估 |
| 6 | 远程 registry（tenthpm 中央仓库） | ❌ 未实现（本地 registry 可用） |
| 7 | 协调：AUDIT-11.4.39（重载运行时 VM/解释器分派不一致） | ⚠️ 待排期 |
| 8 | 错误消息文本本身不冻结（属可改进项），但**错误类别/行号语义**冻结 | 约定 |
