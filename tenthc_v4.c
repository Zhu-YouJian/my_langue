...
FnDef parse_fn(Parser* p) {
    ...
    void* body = Vec_new();
    while (((p->pos < Vec_len(p->tokens)) && (strcmp(((Token*)Vec_get(p->tokens, p->pos))->kind, KIND_RBRACE()) != 0))) {
...
Program parse_program(void* tokens) {
    ...
    int _ppc=0;
    while (1) {
    if (_ppc<8) { void* _tk=Vec_get((p).tokens,(p).pos); printf("DBG pp#%d pos=%lld kind=%s\n",_ppc,(long long)(p).pos,_tk?((Token*)_tk)->kind:"NULL"); fflush(stdout); _ppc++; }
    if (((p).pos >= Vec_len((p).tokens))) {
    break;
    }
