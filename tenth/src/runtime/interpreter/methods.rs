        ...
        match recv {
            Value::Tensor(t) => {
                ...
                match method {
                    ...
                    // 与 VM 端 natives.rs 侧一致；shape_tensor() 保留（f64 张量形式）。
                    "shape" => {
                    ...
                    // QA-20260831（黑板留言 5-②）：to_vec() 双侧注册（此前仅类型
                    // 推断存在，运行时「未知的张量方法」——AUDIT-11.4.12 同款）。
                    // 语义与 VM 端 natives.rs 一致：展平（行主序）为一维 Vec，
                    // 元素按 dtype（f64→Float / f32→Float32）。
                    "to_vec" => {
