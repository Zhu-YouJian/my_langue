        ...
        match receiver {
            Type::Tensor { dtype, dims } => {
                match method {
                    ...
                    // `.shape()`：返回 Vec<i64>——MINOR 兼容新增（v1.0.0 后）。
                    // AUDIT-11.4.12 历史注记：该分支曾被删除，因当时运行时无
                    // `.shape()` native（类型检查通过但运行时崩溃）；现已在
                    // VM（vm/natives.rs）与解释器（interpreter/methods.rs）双侧
                    // 注册，恢复类型推断。`.shape_tensor()`（f64 张量）保留。
