...
mod three_stage {
    ...
    fn run_test() {
        ...
        l2.func_wrap("host", "println", |_: Caller<()>, _: i32| {}).unwrap();
        l2.func_wrap("host", "write_file", |_: Caller<()>, _: i32, _: i32| {}).unwrap();
        l2.func_wrap("host", "read_file", |_: Caller<()>, _: i32| -> i32 { 0 }).unwrap();
        l2.func_wrap("host", "str_add", |_: Caller<()>, _: i32, _: i32| -> i32 { 0 }).unwrap();
        l2.func_wrap("host", "str_eq", |_: Caller<()>, _: i32, _: i32| -> i32 { 0 }).unwrap();
        l2.func_wrap("host", "str_int", |_: Caller<()>, _: i64| -> i32 { 0 }).unwrap();
        l2.func_wrap("host", "tenth_alloc", |_: Caller<()>, _: i32| -> i32 { 0 }).unwrap();
        l2.func_wrap("host", "Vec_new", |_: Caller<()>| -> i64 { 0 }).unwrap();
        l2.func_wrap("host", "Vec_push", |_: Caller<()>, _: i64, _: i64| -> i64 { 0 }).unwrap();
        l2.func_wrap("host", "Vec_len", |_: Caller<()>, _: i64| -> i64 { 0 }).unwrap();
        l2.func_wrap("host", "Vec_get", |_: Caller<()>, _: i64, _: i64| -> i64 { 0 }).unwrap();
        l2.func_wrap("host", "compile_host", |_: Caller<()>, _: i32, _: i32| -> i32 { 0 }).unwrap();
        l2.func_wrap("host", "str_len", |_: Caller<()>, _: i32| -> i32 { 0 }).unwrap();
        l2.func_wrap("host", "str_at", |_: Caller<()>, _: i32, _: i64| -> i32 { 0 }).unwrap();
        l2.func_wrap("host", "str_cmp", |_: Caller<()>, _: i32, _: i32, _: i32| -> i32 { 0 }).unwrap();
        l2.func_wrap("host", "f64_bits", |_: Caller<()>, x: f64| -> i64 { x.to_bits() as i64 }).unwrap();
        l2.func_wrap("host", "str_slice", |_: Caller<()>, _: i32, _: i64, _: i64| -> i32 { 0 }).unwrap();
    ...
    fn three_stage_selfhost() {
        ...
        // The wasmtime path (`three_stage_selfhost_wasmtime`) now runs in the
        // default test suite too — the Vec 写回 bug (main returned len=0) was
        // fixed in `wasmtime_host.rs` via `safe_vec_ptr` (previously the faulty
        // `safe_offset(vec as i32, 1)` guard returned 0 for all real bump offsets).
        // AUDIT.md §六 #5 resolved.
        println!("------ wasmi path ------");
    ...
    /// Wasmtime JIT path for three-stage self-hosting verification.
    /// Was previously `#[ignore]`d because the Vec 写回 host imports in
    /// `wasm/wasmtime_host.rs` used a faulty guard (`safe_offset(vec as i32, 1)`)
    /// that rejected every real bump offset, so main returned a Vec of len=0 and
    /// WASM-B was 0 bytes. Fixed by switching those guards to `safe_vec_ptr`
    /// (only rejects negative/high-bit offsets). Wasmi path already passed; this
    /// validates wasmi/wasmtime semantic parity. AUDIT.md §六 #5 resolved.
    #[test]
