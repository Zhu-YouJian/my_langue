...
>
> **2026-07-02 feat: select 原语引入（第五批，论文 T47/T48/T50 关键能力缺口）**：落实论文披露的 select 原语缺失——阻塞 Huber Loss、可微 masked_fill、True MHA。运行时部 + 编译器部 + 数理部 + 标准库部 + 测试部五阶段联合实施。**数理部方案**：前向 `select(cond, then, else_)` 按 cond>0.5 逐元素选 then/else（NumPy 广播），反向链式法则 `d_then = unbroadcast(grad * cond_mask, then_shape)`、`d_else = unbroadcast(grad * (1-cond_mask), else_shape)`，cond 不可微（mask 节点），shape 规则 = 三输入广播后 shape。**运行时部**：(1) `runtime/autodiff.rs` 新增 `TapeOp::Select` 变体 + `tape.select()` 记录方法 + backward Select 分支——**关键修复**：先创建 dummy input（若 then/else 无 tape_id）再分配 Select 节点 id，保证 `id == self.nodes 索引`不变量（backward 依赖 `self.nodes[loss_node_id]` 与 `node_grads[node.id]`），原顺序错位导致 RefCell already borrowed panic；(2) `runtime/tensor.rs` 新增 `Tensor::select` 静态方法（broadcast 三输入、dtype 提升 f32+f64→f64、逐元素选择）+ `broadcast_shape`/`broadcast_to_owned` 辅助函数；(3) `main.rs::register_natives` 与 `interpreter/natives.rs::call_named_fn` 双侧注册 select native（VM 路径 + 解释器路径），native 内部检查 `vm.recording` → 调 `tape.select` 记录到 tape；(4) **三层白名单同步**——`hir/lower/lower_expr.rs:55` HIR lower 白名单、`runtime/interpreter/mod.rs:421-430` 解释器 Var fallback 白名单、`hir/lower/closures.rs` 闭包自由变量白名单均加入 "select"；(5) `hir/lower/types.rs` 新增 select shape 推断（三输入 broadcast）；(6) `runtime/interpreter/mod.rs` 新增 select FnRef 注册。**标准库部**：(1) `std/nn/activations.th` 新增 `leaky_relu_select(x, negative_slope=0.01)` 与 `leaky_relu_select_default(x)`——用 `sign = x/(|x|+eps)`、`cond = (sign+1)*0.5`、`select(cond, x, neg_slope*x)` 构造可微 leaky_relu；(2) `std/nn/loss.th` 新增 `huber_loss(pred, target, delta=1.0)` 与 `huber_loss_train(pred, target, delta=1.0)`——用 select 实现二次区 `0.5*a²`（|a|≤δ）与线性区 `δ*(|a|-0.5*δ)`（|a|>δ）分段；(3) `std/prelude.th` 同步 leaky_relu_select/huber_loss/huber_loss_train + select 原语说明。**编译器部同步**：`tenthc/hir/lower.th` 白名单加入 "select"（tenthc 不需支持 select 的 autodiff，仅 HIR lower 识别函数名避免误报未定义；bridge.rs 不触及，因 select 是 native 非 HIR 算子）。**测试部** `tests/select_test.rs`（新建）16 个测试：前向基本（test_select_forward_basic）、truthy 阈值（>0.5）、4 个广播用例（cond 标量 true/false、then 标量、混合）、3 个反向梯度（then_grad/else_grad/partial/mixed）、leaky_relu_select 算术对比（正/负区）、huber_loss 前向（二次/线性区）、huber_loss 反向（二次/线性区）、VM 路径（test_select_vm_path——需注册 tensor native 因 parser 将 `tensor[[...]]` 解析为 `Call{func:"tensor", args:[TensorLiteral]}`）。**关键 bug 修复**：(1) tape node id 顺序问题（如上）；(2) leaky_relu 算术对比编码错误——原 `x.relu()+0.01*(-x).relu()` 对 x<0 给正值，修正为 `x.relu()+0.01*(x-x.relu())`；(3) huber_loss 线性区反向——`abs` 方法未接入 autodiff（pre-existing 限制），改用 `a` 直接表达（a>0 时 |a|=a）。**验证**：`select_test` 16/16；`autodiff_test` 52/52；`stdlib_test` 114/114；`vm_autodiff_test` 15/15；自举 `run tenthc/main.th` → `[OK] Full compiler compiled to tenthc_full.wasm`；`selfhost_frontend` 4/4；`parity_test` 129/129。**预存失败（非本次引入，stash 验证）**：`fixpoint_runtime_benchmark`（tenthc WASM 后端对 trivial `fn add` 产出 0 字节，预存 bug）。**改动的 11 个文件**：autodiff.rs、tensor.rs、main.rs、interpreter/mod.rs、interpreter/natives.rs、hir/lower/lower_expr.rs、hir/lower/closures.rs、hir/lower/types.rs、std/nn/activations.th、std/nn/loss.th、std/prelude.th、tenthc/hir/lower.th。新增 1 个测试文件 `tenth/tests/select_test.rs`。**autodiff 算子集扩展**：21→22（新增 TapeOp::Select）。**文档同步**：`能力梳理/能力全梳理.md` Where/Select ❌→✅、22 个算子 backward、Huber Loss 加入损失函数；`MEMO.md` 本条记录。**自举三路径未破坏**。**禁止事项无违反**：仅编辑现有文件 + 新增 1 个测试文件，未破坏已有测试。**遗留**：`abs` 方法未接入 autodiff（pre-existing，huber_loss 线性区 workaround 用 `a` 替代 `abs_a`）；可微 masked_fill 与 True MHA 仍受 Batched Matmul ❌ 限制（见能力全梳理 §5.1）。
>
