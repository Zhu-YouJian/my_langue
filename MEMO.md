...
>
> **当前阶段：v0.3.0 — Phase 7 (标准库) 完成，Phase 8 (自举) 进行中**
...
1. **结构体初始化**：Tenth 不支持 `null`/默认值，结构体每个字段都必须初始化。嵌套结构体需要写几十层嵌套的 `Expr { kind: "", ... }`，代码极其冗长。
2. **泛型类型在表达式中**：`Vec<Token>` 等泛型语法在函数签名的返回值位置触发解析器错误（`<` 被视为泛型参数）。
3. **无 `enum` 元组变体**：只能用 struct 模拟 ADT，不能用 `Some(42)` 这种简洁写法。
4. **无闭包 / 高阶函数**：不能写 `tokens.map(|t| ...)`。

**优先级最高的修复（解锁自举）：**
- [ ] 支持结构体字段默认值或 `Default` trait
- [ ] 修复泛型在返回类型中的解析
- [ ] 支持 `null` 作为结构体字段的占位值
