...
> 数量列格式：`passed/failed/ignored`。"栈溢出" 表示编译通过但运行时触发 Windows STATUS_STACK_OVERFLOW (0xc00000fd)，无法获取具体用例数。**现行统计基线以 `能力梳理/能力全梳理.md` §统计基线为准（**2026-09-17：2608 passed / 0 failed / 23 ignored；149 个测试目标**）**；下表 per-文件 明细为较旧口径的存档记录（统计日期：2026-07-12，--release 模式），可能与合计数字不完全同步（合计见本行下行【总计】）。
...
| 测试目标 | 数量 | 说明 |
|----------|------|------|
| **总计** | **2608 passed / 0 failed / 23 ignored** | 现行基线（2026-09-17，`cargo test --release` 实测，149 个测试目标；需 `RUST_MIN_STACK=33554432`，套件数以实际为准）. 权威合计见 `能力梳理/能力全梳理.md` §统计基线。 |
...
### 11.4.69 ~ 11.4.79（2026-09-17 三批登记）
...
| AUDIT-11.4.72 | **`compile/wasm/compile.rs:22` 无条件 `eprintln!("[WASM] compile …")`** | 工程噪声（stderr 刷屏） | 无条件打印，是**自举 stderr 刷屏**的来源；污染用户与 CI 日志 | ⚠️ 待另案（编译器部，低风险）：改为 verbose 开关或删除 |
...
| 总登记条目数 | **101**（11.1 ×5、11.2 ×5、11.3 ×2、**11.4 ×78**、P 系列 ×11；2026-09-17 总师实测 + 新转入 11.4.60 ~ 11.4.62 + 二批 11.4.63 ~ 11.4.68 + 三批 11.4.69 ~ 11.4.79 + P-11）。**本字段此前长期失实**：原记「55（11.4 ×43）」为 2026-09-08 前旧数，未随 09-08/09-09/09-16/09-17 四个批次更新；且「P 系列 10 项是否计入」原先无定义（`### 11.4.38` 是 P 系列小节标题、不是条目编号）。**P-3=11.4.34、P-4=11.4.32 遗留② 属同项重复登记** ⇒ 去重后**唯一条目 78**；再计入 11.4.60 ~ 11.4.79 与 P-11 后 ⇒ 去重前 **101**、去重后**唯一条目 99** |
