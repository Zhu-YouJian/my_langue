...
| AUDIT-11.4.42 | VM `vm_eq` 对「交换后 Vec 元素」比较不稳定 + VM 索引结果链式方法调用受限（`gsum[0].sum()` 报「没有方法 'sum'」） | M4.3 标准库 AI 生态调研实证发现（2026-08-04 登记；2026-08-31 子项② 已随 QA-20260831 修复，子项① 仍 ⚠️ 待排期） | 两处 VM 限制（解释器路径均正常）：① **`vm_eq` 不稳定**——`v.set(i, v.get(j))` 交换（Fisher-Yates）返回的 Vec，其元素在「双层判定循环 + 后续代码」场景下 `==` 比较错误（实测 probe20/26/29：报告"有重复"，实际排列合法 `[4,0,3,1,2]`）；单层打印 / sum 求和 / 局部变量 vs 字面量比较均稳定；疑似 `set` 写入的 `Value::Shared` 元素在 `vm_eq` 中解析不稳定（与 AUDIT-11.4.24 同域）。② **索引结果链式方法**——`gsum[0].sum()` 报「张量没有方法 'sum'」（索引表达式结果的方法分派缺失），`gsum[0][0]` 双重索引正常（probe35）。**性质**：① 静默错值方向（比较错判）；② 能力缺口。**M4.3 规避**：测试/标准库用 sum（0..n-1 排列恒 n(n-1)/2）与 len 断言，规避元素间 `==`；张量取值用 `[i][j]` 双重索引。 | ⚠️ 部分修复（2026-08-31 QA-20260831，运行时部 + 文档部实测）：**子项②（索引结果链式方法）✅ 已修复**——VM opcode 36 `IndexGet` 解包 Shared/MutRef（`runtime/vm/execute.rs`）后，索引表达式结果可直接链式方法调用；2026-08-31 文档部实测最小用例 `gsum[0].sum()`（gsum 为 2×2 张量）VM=解释器双路径均输出 3.0，`gsum[0][0]` 双重索引维持正常；instance_batch 的 bintree（无法索引）/queue 用例同根因转正常守护（VM_GAP 清单清空）。**子项①（vm_eq 交换后 Vec 元素比较不稳定）未修，仍 ⚠️ 待排期**——M61 sampler 测试注释仍注明此限制存在；待排查 `vm_eq`（`runtime/vm/natives.rs`）对 Shared 包裹元素在复杂控制流下的稳定性（对齐解释器 `values_eq` 的解包语义）；M4.3 规避方案（sum/len 断言替代元素间 `==`）继续有效。 |
...
| 总登记条目数 | 46（11.1 ×5、11.2 ×4、11.3 ×2、11.4 ×35） |
