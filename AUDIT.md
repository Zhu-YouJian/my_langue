...
| P-3 | AUDIT-11.4.34（match tuple 模式 + guard） | ⚠️ 部分修复（M2-A5）：② JIT 编译 panic 逃逸已根治（context.rs 整个编译链路 catch_unwind）；① **VM guard 回退错乱待排期**——guard 失败后不尝试下一条 tuple 臂直接落 wildcard（静默错值方向） | VM lowering 预存缺陷非 M2 引入；一致性套件已用「单 guard 臂 + wildcard」稳定形态 | M3 运行时部排期修复 VM match lowering 的 guard 回退逻辑 |
