...
| AUDIT-11.4.42 | VM `vm_eq` 对「交换后 Vec 元素」比较不稳定 + VM 索引结果链式方法调用受限（`gsum[0].sum()` 报「没有方法 'sum'」） | M4.3 标准库 AI 生态调研实证发现（2026-08-04 登记，⚠️ 待排期） | 两处 VM 限制（解释器路径均正常）：① **`vm_eq` 不稳定**——`v.set(i, v.get(j))` 交换（Fisher-Yates）返回的 Vec，其元素在「双层判定循环 + 后续代码」场景下 `==` 比较错误（实测 probe20/26/29：报告"有重复"，实际排列合法 `[4,0,3,1,2]`）；单层打印 / sum 求和 / 局部变量 vs 字面量比较均稳定；疑似 `set` 写入的 `Value::Shared` 元素在 `vm_eq` 中解析不稳定（与 AUDIT-11.4.24 同域）。② **索引结果链式方法**——`gsum[0].sum()` 报「张量没有方法 'sum'」（索引表达式结果的方法分派缺失），`gsum[0][0]` 双重索引正常（probe35）。**性质**：① 静默错值方向（比较错判）；② 能力缺口。**M4.3 规避**：测试/标准库用 sum（0..n-1 排列恒 n(n-1)/2）与 len 断言，规避元素间 `==`；张量取值用 `[i][j]` 双重索引。 | ⚠️ 待排期（运行时部，VM natives/execute）：① 排查 `vm_eq`（`runtime/vm/natives.rs`）对 Shared 包裹元素在复杂控制流下的稳定性（对齐解释器 `values_eq` 的解包语义）；② VM 张量方法分派补「索引表达式结果」的 MethodCall 支持（`call_method_priv` 对 IndexGet 结果的处理）。 |

| AUDIT-11.4.43 | JIT 对 **Union 字段修改**（`FieldAssign` 到 Union 变量，如 `n3.integer = 100`）触发 Cranelift 低化 `TryFromIntError` panic（`cranelift-codegen-0.115.1 isa/x64/lower/isle.rs:75`）——stderr 打印 panic 噪音，功能靠 catch_unwind fallback 兜底 | M5.2 大规模回归实例批量扫描实证发现（2026-08-04 登记，⚠️ 待排期修复） | `Tenth实例/Union类型/union_demo.th`（默认路径）每次运行 stderr 都打印 `thread 'tenth-main' panicked at ...cranelift-codegen...isle.rs:75:5: called 'Result::unwrap()' on an 'Err' value: TryFromIntError(())`，随后程序正常输出（exit=0）——panic 发生在 **Cranelift 低化（finalize_definitions）**，已被 M2-A5 的 `catch_unwind`（`compile/jit/context.rs`，translate+finalize+get_finalized_function 全链路）捕获 → 转 Err → JIT→VM fallback → 功能正确。**最小复现**（`.trae/tmp/union_min2.th`）：`union Number { integer: i64, float: f64 } fn main() { let mut n3 = Number { integer: 1 }; n3.integer = 100; println(n3.integer); }`——**触发**；对照 `union_min1.th`（只构造 + 字段访问，无修改）**不触发** → 根因锁定在 Union `FieldAssign`（bytecode `StoreField` + `Store` 写回）的 JIT 发射路径。**性质**：非崩溃（fallback 兜底）、非静默错值（输出正确），但 stderr 被 panic 噪音污染（脚本解析 stderr 会误判失败）；与 AUDIT-11.4.34②同族（Cranelift 低化 TryFromIntError，M2-A5 只根治了"逃逸"，未消除"触发"）。**M5.2 守护**：`instance_batch_test.rs` 将 union_demo 归入 KNOWN_JIT_PANIC_STDERR 分类——断言功能正确（exit=0 + 输出含 '100'），容忍 stderr panic 噪音，并注释关联本 AUDIT。 | ⚠️ 待排期（编译器部/运行时部，JIT translator）：定位 Union `FieldAssign`（StoreField+Store 写回）在 translator 的发射——排查是否 `Store` 对 Union 结果/标量槽的不当处理（`local_scalars`/`stack_scalars` 跟踪或 iconst 参数越界）触发 Cranelift 低化失败；修复后移除 KNOWN_JIT_PANIC_STDERR 分类改为普通断言（stderr 禁 panic）。 |
