...
void* lexer_tokenize(Lexer* lexer) {
    printf("DBG tokenize: enter, src=%p\n", (void*)lexer->source); fflush(stdout);
...
Program parse_program(void* tokens) {
    printf("DBG parse_program: enter, toks=%p len=%lld\n", tokens, (long long)Vec_len(tokens)); fflush(stdout);
    ...
    ((strcmp(((Expr*)Vec_get((p).tokens, (p).pos))->kind, KIND_STRUCT()) == 0) ? Vec_push(structs, ({ StructDef _t = parse_struct(&p); &_t; })) : ((strcmp(((Expr*)Vec_get((p).tokens, (p).pos))->kind, KIND_FN()) == 0) ? Vec_push(fns, ({ FnDef _t = parse_fn(&p); &_t; })) : Vec_push(main_stmts, ({ Stmt _t = parse_stmt(&p); &_t; }))));
    }
...
const char* compile(const char* src) {
    printf("DBG compile: enter, srclen=%d\n", src ? (int)strlen(src) : -1); fflush(stdout);
    /* Let lex declared=TypeParam { name: "Lexer" } val_ty=TypeParam { name: "Lexer" } */
    Lexer lex = lexer_new(src);
    printf("DBG compile: lex created, calling tokenize\n"); fflush(stdout);
    /* Let tokens declared=TypeParam { name: "Vec" } val_ty=TypeParam { name: "Vec" } */
    void* tokens = lexer_tokenize(&lex);
    printf("DBG compile: tokenize done, toks len=%lld\n", (long long)Vec_len(tokens)); fflush(stdout);
