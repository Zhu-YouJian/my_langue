...
int64_t char_to_digit(const char* ch);
int64_t parse_int(const char* s);
double parse_float(const char* whole, const char* frac);
...
int64_t char_to_digit(const char* ch) {
    ...
    int32_t tmp_0 = (str_eq(ch, "0") ? 0 : (str_eq(ch, "1") ? 1 : (str_eq(ch, "2") ? 2 : (str_eq(ch, "3") ? 3 : (str_eq(ch, "4") ? 4 : (str_eq(ch, "5") ? 5 : (str_eq(ch, "6") ? 6 : (str_eq(ch, "7") ? 7 : (str_eq(ch, "8") ? 8 : (str_eq(ch, "9") ? 9 : 0))))))))));
    return (int64_t)tmp_0;
}

int64_t parse_int(const char* s) {
    /* Let result declared=Base(I32) val_ty=Base(I32) */
    int32_t result = 0;
    /* Let i declared=Base(I32) val_ty=Base(I32) */
    int32_t i = 0;
    /* Let len declared=Base(I64) val_ty=Base(I64) */
    int64_t len = ((int64_t)strlen(s));
    while ((i < len)) {
    /* Let ch declared=Unknown val_ty=Unknown */
    void* ch = str_at(s, i);
...
double parse_float(const char* whole, const char* frac) {
    /* Let w declared=Base(I64) val_ty=Base(I64) */
    int64_t w = parse_int(whole);
    /* Let f declared=Base(I64) val_ty=Base(I64) */
    int64_t f = parse_int(frac);
    /* Let div declared=Base(F64) val_ty=Base(F64) */
    double div = 1.0000000000;
    /* Let i declared=Base(I32) val_ty=Base(I32) */
    int32_t i = 0;
    while ((i < ((int64_t)strlen(frac)))) {
    div = (div * 10.0000000000);
    i = (i + 1);
    0;
    }
    return (double)(w + (f / div));
}
...
Token lexer_next(Lexer* lexer) {
    ...
    if (str_eq(ch, "")) {
    return (Token)((Token){ .kind = 61, .span = span, .ival = 0, .sval = "" });
    }
    if (is_digit(ch)) {
    /* Let int_part declared=Base(Str) val_ty=Base(Str) */
    const char* int_part = "";
    /* Let frac_part declared=Base(Str) val_ty=Base(Str) */
    const char* frac_part = "";
    /* Let is_float declared=Base(Bool) val_ty=Base(Bool) */
    bool is_float = false;
    while (is_digit(ch)) {
    int_part = str_add(int_part, ch);
    ch = lexer_advance(lexer);
    ch = lexer_peek(lexer);
    0;
    }
    ...
    ch = lexer_peek(lexer);
    while (is_digit(ch)) {
    frac_part = str_add(frac_part, ch);
    ch = lexer_advance(lexer);
    ch = lexer_peek(lexer);
    0;
    }
    ...
    if (is_float) {
    return (Token)((Token){ .kind = 1, .span = span, .ival = 0, .sval = int_part });
    }
    /* Let iv declared=Base(I64) val_ty=Base(I64) */
    int64_t iv = parse_int(int_part);
    return (Token)((Token){ .kind = 0, .span = span, .ival = iv, .sval = "" });
    }
    if (is_alpha(ch)) {
    ...
    if (str_eq(ident, "fn")) {
    return (Token)((Token){ .kind = 4, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "let")) {
    return (Token)((Token){ .kind = 5, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "mut")) {
    return (Token)((Token){ .kind = 6, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "if")) {
    return (Token)((Token){ .kind = 7, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "else")) {
    return (Token)((Token){ .kind = 8, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "match")) {
    return (Token)((Token){ .kind = 9, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "return")) {
    return (Token)((Token){ .kind = 10, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "while")) {
    return (Token)((Token){ .kind = 11, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "for")) {
    return (Token)((Token){ .kind = 12, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "loop")) {
    return (Token)((Token){ .kind = 13, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "break")) {
    return (Token)((Token){ .kind = 14, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "continue")) {
    return (Token)((Token){ .kind = 15, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "struct")) {
    return (Token)((Token){ .kind = 16, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "enum")) {
    return (Token)((Token){ .kind = 17, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "impl")) {
    return (Token)((Token){ .kind = 18, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "trait")) {
    return (Token)((Token){ .kind = 19, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "use")) {
    return (Token)((Token){ .kind = 20, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "mod")) {
    return (Token)((Token){ .kind = 21, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "true")) {
    return (Token)((Token){ .kind = 22, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "false")) {
    return (Token)((Token){ .kind = 23, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "move")) {
    return (Token)((Token){ .kind = 24, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ident, "self")) {
    return (Token)((Token){ .kind = 25, .span = span, .ival = 0, .sval = "" });
    }
    return (Token)((Token){ .kind = 3, .span = span, .ival = 0, .sval = ident });
    }
    if (str_eq(ch, "\"")) {
    lexer_advance(lexer);
    ...
    return (Token)((Token){ .kind = 2, .span = span, .ival = 0, .sval = s });
    }
    lexer_advance(lexer);
    if (str_eq(ch, "(")) {
    return (Token)((Token){ .kind = 47, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ch, ")")) {
    return (Token)((Token){ .kind = 48, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ch, "{")) {
    return (Token)((Token){ .kind = 49, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ch, "}")) {
    return (Token)((Token){ .kind = 50, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ch, "[")) {
    return (Token)((Token){ .kind = 51, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ch, "]")) {
    return (Token)((Token){ .kind = 52, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ch, ",")) {
    return (Token)((Token){ .kind = 53, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ch, ";")) {
    return (Token)((Token){ .kind = 54, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ch, ".")) {
    if (str_eq(lexer_peek(lexer), ".")) {
    lexer_advance(lexer);
    return (Token)((Token){ .kind = 57, .span = span, .ival = 0, .sval = "" });
    }
    return (Token)((Token){ .kind = 56, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ch, ":")) {
    if (str_eq(lexer_peek(lexer), ":")) {
    lexer_advance(lexer);
    return (Token)((Token){ .kind = 60, .span = span, .ival = 0, .sval = "" });
    }
    return (Token)((Token){ .kind = 55, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ch, "+")) {
    if (str_eq(lexer_peek(lexer), "=")) {
    lexer_advance(lexer);
    return (Token)((Token){ .kind = 43, .span = span, .ival = 0, .sval = "" });
    }
    return (Token)((Token){ .kind = 26, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ch, "-")) {
    if (str_eq(lexer_peek(lexer), ">")) {
    lexer_advance(lexer);
    return (Token)((Token){ .kind = 58, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(lexer_peek(lexer), "=")) {
    lexer_advance(lexer);
    return (Token)((Token){ .kind = 44, .span = span, .ival = 0, .sval = "" });
    }
    return (Token)((Token){ .kind = 27, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ch, "*")) {
    if (str_eq(lexer_peek(lexer), "=")) {
    lexer_advance(lexer);
    return (Token)((Token){ .kind = 45, .span = span, .ival = 0, .sval = "" });
    }
    return (Token)((Token){ .kind = 28, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ch, "/")) {
    if (str_eq(lexer_peek(lexer), "=")) {
    lexer_advance(lexer);
    return (Token)((Token){ .kind = 46, .span = span, .ival = 0, .sval = "" });
    }
    return (Token)((Token){ .kind = 29, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ch, "%")) {
    return (Token)((Token){ .kind = 30, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ch, "=")) {
    if (str_eq(lexer_peek(lexer), "=")) {
    lexer_advance(lexer);
    return (Token)((Token){ .kind = 31, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(lexer_peek(lexer), ">")) {
    lexer_advance(lexer);
    return (Token)((Token){ .kind = 59, .span = span, .ival = 0, .sval = "" });
    }
    return (Token)((Token){ .kind = 42, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ch, "!")) {
    if (str_eq(lexer_peek(lexer), "=")) {
    lexer_advance(lexer);
    return (Token)((Token){ .kind = 32, .span = span, .ival = 0, .sval = "" });
    }
    return (Token)((Token){ .kind = 39, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ch, "<")) {
    if (str_eq(lexer_peek(lexer), "=")) {
    lexer_advance(lexer);
    return (Token)((Token){ .kind = 35, .span = span, .ival = 0, .sval = "" });
    }
    return (Token)((Token){ .kind = 33, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ch, ">")) {
    if (str_eq(lexer_peek(lexer), "=")) {
    lexer_advance(lexer);
    return (Token)((Token){ .kind = 36, .span = span, .ival = 0, .sval = "" });
    }
    return (Token)((Token){ .kind = 34, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ch, "&")) {
    if (str_eq(lexer_peek(lexer), "&")) {
    lexer_advance(lexer);
    return (Token)((Token){ .kind = 37, .span = span, .ival = 0, .sval = "" });
    }
    return (Token)((Token){ .kind = 40, .span = span, .ival = 0, .sval = "" });
    }
    if (str_eq(ch, "|")) {
    if (str_eq(lexer_peek(lexer), "|")) {
    lexer_advance(lexer);
    return (Token)((Token){ .kind = 38, .span = span, .ival = 0, .sval = "" });
    }
    return (Token)((Token){ .kind = 41, .span = span, .ival = 0, .sval = "" });
    }
    return (Token)((Token){ .kind = 3, .span = span, .ival = 0, .sval = "" });
}
...
int64_t parse_primary(Parser* p) {
    ...
    /* Let match_res_1 declared=Base(I64) val_ty=Base(I64) */
    int64_t match_res_1 = 0;
    if ((match_disc_0 == 0)) {
    /* Let n declared=Base(I64) val_ty=Base(I64) */
    int64_t n = (tok).ival;
    /* Let idx declared=Base(I64) val_ty=Base(I64) */
    int64_t idx = add_expr(p, "int");
    ((Token*)Vec_get(p->expr_nodes, (idx - 1)))->ival = n;
    match_res_1 = idx;
    } else {
    if ((match_disc_0 == 1)) {
    /* Let n declared=Base(I64) val_ty=Base(I64) */
    ...
    if ((match_disc_0 == 2)) {
    /* Let s declared=Base(Str) val_ty=Base(Str) */
    const char* s = (tok).sval;
    /* Let idx declared=Base(I64) val_ty=Base(I64) */
    int64_t idx = add_expr(p, "str");
    ((Token*)Vec_get(p->expr_nodes, (idx - 1)))->sval = s;
    match_res_1 = idx;
    } else {
    if ((match_disc_0 == 22)) {
    /* Let idx declared=Base(I64) val_ty=Base(I64) */
    int64_t idx = add_expr(p, "bool");
    ((Token*)Vec_get(p->expr_nodes, (idx - 1)))->ival = 1;
    match_res_1 = idx;
    } else {
    if ((match_disc_0 == 23)) {
    /* Let idx declared=Base(I64) val_ty=Base(I64) */
    int64_t idx = add_expr(p, "bool");
    ((Token*)Vec_get(p->expr_nodes, (idx - 1)))->ival = 0;
    match_res_1 = idx;
    } else {
    if ((match_disc_0 == 3)) {
    /* Let s declared=Base(Str) val_ty=Base(Str) */
    ...
    /* Let idx declared=Base(I64) val_ty=Base(I64) */
    int64_t idx = add_expr(p, "ident");
    ((Token*)Vec_get(p->expr_nodes, (idx - 1)))->sval = "self";
    match_res_1 = idx;
    } else {
    if ((match_disc_0 == 47)) {
    ...
    ((Token*)Vec_get(p->expr_nodes, (idx - 1)))->sval = "-";
    ((Expr*)Vec_get(p->expr_nodes, (idx - 1)))->left = inner;
    match_res_1 = idx;
    } else {
    if ((match_disc_0 == 39)) {
...
int64_t parse_postfix(Parser* p) {
    ...
    ((Token*)Vec_get(p->expr_nodes, (expr_idx - 1)))->kind = "call";
    ...
    /* Let match_res_3 declared=Base(I64) val_ty=Base(I64) */
    int64_t match_res_3 = 0;
    if ((match_disc_2 == 3)) {
    field_name = (field_tok).sval;
    }
    match_res_3;
    /* Let idx declared=Base(I64) val_ty=Base(I64) */
    int64_t idx = add_expr(p, "field");
    ((Expr*)Vec_get(p->expr_nodes, (idx - 1)))->left = expr_idx;
    ((Token*)Vec_get(p->expr_nodes, (idx - 1)))->sval = field_name;
    expr_idx = idx;
    } else {
    break;
    }
...
int64_t parse_arg_list(Parser* p) {
    ...
    /* Let match_res_3 declared=TypeParam { name: "Token" } val_ty=TypeParam { name: "Token" } */
    Token match_res_3 = (Token){0};
    if ((match_disc_2 == 53)) {
    } else {
    break;
    }
...
const char* token_to_binop_str(Token tok) {
    /* Let match_disc_0 declared=Base(I64) val_ty=TypeParam { name: "TokenKind" } */
    int64_t match_disc_0 = (tok).kind;
    /* Let match_res_1 declared=Base(Str) val_ty=Base(Str) */
    const char* match_res_1 = 0;
    if ((match_disc_0 == 26)) {
    } else {
    if ((match_disc_0 == 27)) {
    } else {
...
int64_t parse_binary(Parser* p, int64_t min_prec) {
    ...
    ((Token*)Vec_get(p->expr_nodes, (idx - 1)))->sval = op;
...
int64_t parse_match_expr(Parser* p) {
    ...
    /* Let match_res_5 declared=Base(I64) val_ty=Base(I64) */
    int64_t match_res_5 = 0;
    if ((match_disc_4 == 60)) {
    parser_advance(p);
    ...
    /* Let match_res_7 declared=Base(I64) val_ty=Base(I64) */
    int64_t match_res_7 = 0;
    if ((match_disc_6 == 3)) {
    pat_name = (variant_tok).sval;
    }
    ...
    /* Let match_res_9 declared=TypeParam { name: "Token" } val_ty=TypeParam { name: "Token" } */
    Token match_res_9 = (Token){0};
    if ((match_disc_8 == 53)) {
    }
    match_res_9;
    }
...
int64_t parse_stmt(Parser* p) {
    ...
    /* Let match_res_7 declared=TypeParam { name: "Token" } val_ty=TypeParam { name: "Token" } */
    Token match_res_7 = (Token){0};
    if ((match_disc_6 == 55)) {
    parser_advance(p);
    }
    match_res_7;
    ...
    /* Let match_res_9 declared=Base(I64) val_ty=Base(I64) */
    int64_t match_res_9 = 0;
    if ((match_disc_8 == 42)) {
    parser_advance(p);
    ...
    ((Param*)Vec_get(p->stmt_nodes, (idx - 1)))->name = name;
    ...
    ((Stmt*)Vec_get(p->stmt_nodes, (idx - 1)))->body_start = body_start;
    ((Stmt*)Vec_get(p->stmt_nodes, (idx - 1)))->body_count = body_count;
    match_res_1 = idx;
    } else {
    if ((match_disc_0 == 14)) {
    parser_advance(p);
...
FnDef parse_fn(Parser* p) {
    ...
    /* Let match_res_11 declared=Base(I64) val_ty=Base(I64) */
    int64_t match_res_11 = 0;
    if ((match_disc_10 == 3)) {
    return_type = (ret_tok).sval;
    }
...
EnumDef parse_enum_def(Parser* p) {
    ...
    /* Let match_res_5 declared=TypeParam { name: "Token" } val_ty=TypeParam { name: "Token" } */
    Token match_res_5 = (Token){0};
    if ((match_disc_4 == 47)) {
    parser_advance(p);
    while (1) {
    /* Let t3 declared=TypeParam { name: "Token" } val_ty=TypeParam { name: "Token" } */
    ...
    /* Let match_res_11 declared=TypeParam { name: "Token" } val_ty=TypeParam { name: "Token" } */
    Token match_res_11 = (Token){0};
    if ((match_disc_10 == 53)) {
    } else {
    break;
    }
    ...
    /* Let match_res_13 declared=TypeParam { name: "Token" } val_ty=TypeParam { name: "Token" } */
    Token match_res_13 = (Token){0};
    if ((match_disc_12 == 53)) {
    } else {
    break;
    }
...
bool is_end_of_block(Token tok) {
    /* Let match_disc_0 declared=Base(I64) val_ty=TypeParam { name: "TokenKind" } */
    int64_t match_disc_0 = (tok).kind;
    /* Let match_res_1 declared=Base(Bool) val_ty=Base(Bool) */
    bool match_res_1 = 0;
    if ((match_disc_0 == 50)) {
    match_res_1 = true;
    } else {
...
const char* cgen_expr(Program* prog, void* exprs, void* stmts, int64_t idx) {
    ...
    if (str_eq(((Token*)e)->kind, "int")) {
    return (const char*)int_to_str(((Token*)e)->ival);
    }
    if (str_eq(((Token*)e)->kind, "float")) {
    return (const char*)float_to_str(((Expr*)e)->fval);
    }
    if (str_eq(((Token*)e)->kind, "str")) {
    return (const char*)str_add(str_add("\"", ((Token*)e)->sval), "\"");
    }
    if (str_eq(((Token*)e)->kind, "bool")) {
    if ((((Token*)e)->ival != 0)) {
    ...
    if (str_eq(((Token*)e)->kind, "ident")) {
    return (const char*)((Token*)e)->sval;
    }
    if (str_eq(((Token*)e)->kind, "binary")) {
    ...
    return (const char*)str_add(str_add(str_add(str_add(str_add(str_add("(", left), " "), ((Token*)e)->sval), " "), right), ")");
    }
    if (str_eq(((Token*)e)->kind, "unary")) {
    /* Let inner declared=Base(Str) val_ty=Base(Str) */
    const char* inner = cgen_expr(prog, exprs, stmts, ((Expr*)e)->left);
    return (const char*)str_add(str_add(str_add("(", ((Token*)e)->sval), inner), ")");
    }
    if (str_eq(((Token*)e)->kind, "call")) {
    /* Let args declared=Base(Str) val_ty=Base(Str) */
    const char* args = "";
    /* Let i declared=Base(I32) val_ty=Base(I32) */
    int32_t i = 0;
    while ((i < ((Expr*)e)->arg_count)) {
    if ((i > 0)) {
    args = str_add(args, ", ");
    }
    /* Let arg_idx declared=Unknown val_ty=Unknown */
    int64_t arg_idx = (((Expr*)e)->arg_start + i);
    args = str_add(args, cgen_expr(prog, exprs, stmts, arg_idx));
    i = (i + 1);
    0;
    }
    return (const char*)str_add(str_add(str_add(((Token*)e)->sval, "("), args), ")");
    }
    if (str_eq(((Token*)e)->kind, "field")) {
    /* Let target declared=Base(Str) val_ty=Base(Str) */
    const char* target = cgen_expr(prog, exprs, stmts, ((Expr*)e)->left);
    return (const char*)str_add(str_add(str_add("(", target), ")."), ((Token*)e)->sval);
    }
    if (str_eq(((Token*)e)->kind, "ref")) {
    /* Let inner declared=Base(Str) val_ty=Base(Str) */
    const char* inner = cgen_expr(prog, exprs, stmts, ((Expr*)e)->left);
    return (const char*)str_add("&", inner);
    }
    if (str_eq(((Token*)e)->kind, "deref")) {
    /* Let inner declared=Base(Str) val_ty=Base(Str) */
    const char* inner = cgen_expr(prog, exprs, stmts, ((Expr*)e)->left);
    return (const char*)str_add(str_add("(*", inner), ")");
    }
    if (str_eq(((Token*)e)->kind, "block")) {
...
const char* cgen_stmt(Program* prog, void* exprs, void* stmts, int64_t idx) {
    ...
    if (str_eq(((Token*)s)->kind, "let")) {
    /* Let init declared=Base(Str) val_ty=Base(Str) */
    const char* init = cgen_expr(prog, exprs, stmts, ((Stmt*)s)->expr_idx);
    return (const char*)str_add(str_add(str_add(str_add("int ", ((Param*)s)->name), " = "), init), ";\n");
    }
    if (str_eq(((Token*)s)->kind, "return")) {
    ...
    if (str_eq(((Token*)s)->kind, "expr")) {
    /* Let val declared=Base(Str) val_ty=Base(Str) */
    const char* val = cgen_expr(prog, exprs, stmts, ((Stmt*)s)->expr_idx);
    return (const char*)str_add(val, ";\n");
    }
    if (str_eq(((Token*)s)->kind, "while")) {
    /* Let cond declared=Base(Str) val_ty=Base(Str) */
    ...
    /* Let i declared=Base(I32) val_ty=Base(I32) */
    int32_t i = 0;
    while ((i < ((Stmt*)s)->body_count)) {
    /* Let stmt_idx declared=Unknown val_ty=Unknown */
    int64_t stmt_idx = (((Stmt*)s)->body_start + i);
    ...
    if (str_eq(((Token*)s)->kind, "break")) {
    return (const char*)"break;\n";
    }
    if (str_eq(((Token*)s)->kind, "continue")) {
...
const char* cgen_struct(StructDef* s) {
    ...
    out = str_add(str_add(str_add(out, "    int "), ((Param*)f)->name), ";\n");
    i = (i + 1);
    0;
    }
